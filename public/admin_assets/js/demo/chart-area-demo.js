...
fetch("/api/weekly-earnings")
    .then((response) => response.json())
    .then((data) => {
        const earnings = data.weekly_earnings || 0;
        document.getElementById(
            "weeklyEarnings"
        ).textContent = `Rp. ${earnings}`;
    })
    .catch((error) => {
...
fetch("/api/daily-earnings")
    .then((response) => response.json())
    .then((data) => {
        const dailyEarnings = data.daily_earnings || 0;
        document.getElementById(
            "dailyEarnings"
        ).textContent = `Rp. ${dailyEarnings}`;
    })
    .catch((error) => {
...
fetch("/api/buyyer-earnings")
    ...
    .then((data) => {
        document.getElementById(
            "buyyerEarnings"
        ).textContent = `${data.total_purchases}`;
    })
    .catch((error) => {
        console.error("Error fetching total products:", error);
        document.getElementById("buyyerEarnings").textContent =
            "Error loading data";
    });
